...
class BotRuntime:
    ...
    async def run(self) -> None:
        ...
        try:
            await asyncio.gather(self.auto_signal_loop(), self.result_loop())
        finally:
        ...
        while True:
            ...
            for item in enabled_symbols():
                try:
                    ...
                    logger.exception("نماد رد شد: %s - %s", item.get("name"), exc)
                    continue
    ...
    async def process_symbol(self, item: dict[str, object]) -> None:
        ...
        toobit_symbol = str(item["toobit_symbol"])
        ...
        if signal.signal_type == "رئال":
            result = await asyncio.to_thread(self.executor.open_real_position, signal, self.settings.trade_amount_usdt, self.settings.leverage)
            if not result.opened:
                logger.warning("پوزیشن رئال باز نشد: %s %s", toobit_symbol, result.reason)
                return
    ...
    async def result_loop(self) -> None:
        while True:
            ...
            await asyncio.sleep(self.settings.result_interval_seconds)
