...
def main() -> None:
    ...
    storage = Storage()
    try:
        rebuilt = storage.rebuild_adaptive_fix_profiles_from_history()
        if rebuilt:
            LOGGER.info("adaptive fix learning rebuilt from %s historical signals", rebuilt)
    except Exception as exc:
        LOGGER.warning("adaptive fix rebuild skipped: %s", exc)
    okx = OkxDataClient()
    toobit = get_client()
