"""تنظیمات اصلی ربات اسکالپ ۵ دقیقه‌ای فقط شورت v16."""
...
# ورود و خروج
# -----------------------------
FIXED_TP_PERCENT = _get_float("FIXED_TP_PERCENT", 0.55)
FIXED_SL_PERCENT = _get_float("FIXED_SL_PERCENT", 0.35)
MIN_SIGNAL_SCORE = 80
...
# v16: ربات فقط شکارچی شورت ۵ دقیقه‌ای است. لانگ اصلاً سیگنال نمی‌دهد.
SHORT_ONLY_MODE = _get_bool("SHORT_ONLY_MODE", True)
ALLOW_LONG_SIGNALS = _get_bool("ALLOW_LONG_SIGNALS", False)
ALLOW_SHORT_SIGNALS = _get_bool("ALLOW_SHORT_SIGNALS", True)
SIGNAL_MAX_HOLD_MINUTES = _get_int("SIGNAL_MAX_HOLD_MINUTES", 60)
ENTRY_CONFIRM_TIMEFRAME = _get_env("ENTRY_CONFIRM_TIMEFRAME", "15m")
ENTRY_CONFIRM_CANDLE_LIMIT = _get_int("ENTRY_CONFIRM_CANDLE_LIMIT", 120)
REQUIRE_CUSTOM_PROFILE_FOR_SIGNAL = _get_bool("REQUIRE_CUSTOM_PROFILE_FOR_SIGNAL", True)

# -----------------------------
# فیلتر بازه‌ای بدون امتیاز
# -----------------------------
# بازار فقط سه حالت دارد: BUY، SELL، RANGE. در RANGE هیچ سیگنالی صادر نمی‌شود.
MARKET_TREND_TIMEFRAMES = ("1H",)
...
# -----------------------------
# فیلتر حمایت/مقاومت برای اسکالپ شورت
# -----------------------------
SR_FILTER_ENABLED = _get_bool("SR_FILTER_ENABLED", True)
SR_CANDLE_LIMIT_1H = _get_int("SR_CANDLE_LIMIT_1H", 120)
SR_CANDLE_LIMIT_4H = _get_int("SR_CANDLE_LIMIT_4H", 80)
SR_MIN_REACTION_PERCENT = _get_float("SR_MIN_REACTION_PERCENT", 0.40)
SR_ZONE_WIDTH_PERCENT = _get_float("SR_ZONE_WIDTH_PERCENT", 0.20)
SR_MIN_TOUCHES = _get_int("SR_MIN_TOUCHES", 2)
SR_MIN_STRENGTH = _get_float("SR_MIN_STRENGTH", 5.5)
SR_PIVOT_SWING_CANDLES = _get_int("SR_PIVOT_SWING_CANDLES", 2)
SR_REACTION_LOOKAHEAD_CANDLES_1H = _get_int("SR_REACTION_LOOKAHEAD_CANDLES_1H", 4)
SR_REACTION_LOOKAHEAD_CANDLES_4H = _get_int("SR_REACTION_LOOKAHEAD_CANDLES_4H", 3)
...
ROLLING_OPTIMIZER_TARGET_MOVE_PERCENT = _get_float("ROLLING_OPTIMIZER_TARGET_MOVE_PERCENT", FIXED_TP_PERCENT)
ROLLING_OPTIMIZER_ADVERSE_PERCENT = _get_float("ROLLING_OPTIMIZER_ADVERSE_PERCENT", FIXED_SL_PERCENT)
ROLLING_OPTIMIZER_MAX_HOLD_CANDLES = _get_int("ROLLING_OPTIMIZER_MAX_HOLD_CANDLES", 12)
ROLLING_OPTIMIZER_MAX_END_CANDLES = _get_int("ROLLING_OPTIMIZER_MAX_END_CANDLES", 18)
ROLLING_OPTIMIZER_REVERSAL_PERCENT = _get_float("ROLLING_OPTIMIZER_REVERSAL_PERCENT", 0.25)
...
ROLLING_OPTIMIZER_MIN_GOOD_MOVES = _get_int("ROLLING_OPTIMIZER_MIN_GOOD_MOVES", 16)
...
ROLLING_OPTIMIZER_SHORT_RSI_CEIL = _get_float("ROLLING_OPTIMIZER_SHORT_RSI_CEIL", 60.0)
