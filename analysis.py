...
- تحلیل اصلی با EMA / RSI / MACD / MACD Histogram و زیرشاخه‌هایشان؛ 5M وزن بیشتری برای ورود سریع دارد
- ورود مستقیم، ساده، سریع‌تر، قابل دیباگ، شبیه ربات اولیه
...
def simple_classic_score(symbol: str, df_4h: pd.DataFrame, df_1h: pd.DataFrame, df_30m: pd.DataFrame, df_15m: pd.DataFrame, df_5m: pd.DataFrame) -> Dict:
    ...
    ema_tf_weights = {
        # Faster-direction setup: 4H is background only, 1H/15M define direction,
        # and 5M has stronger influence for quicker entries.
        "4H": 6,
        "1H": 15,
        "30M": 10,
        "15M": 18,
    ...
    # Distance from EMA20 is kept for output/debug only.
    # It does not block entries here; speed of direction detection is prioritized.
    too_late_entry = False
    long_reasons.append(f"فاصله از EMA20 برای بررسی: {round(dist_15, 2)} ATR")
    short_reasons.append(f"فاصله از EMA20 برای بررسی: {round(dist_15, 2)} ATR")
    ...
    # 4) ADX: below 20 is rejected; above 20 is accepted; above 25 gets a positive score.
    adx_valid = adx_15 >= 20
    if adx_15 >= 35:
        long_score += 8
        short_score += 8
    ...
    # 5) VWAP: soft confirmation only (+4/-4), not a hard filter.
    if last_15["close"] > last_15["vwap"]:
        long_score += 4
        short_score -= 4
        long_reasons.append("VWAP موافق لانگ؛ +4")
        short_reasons.append("VWAP مخالف شورت؛ -4")
    elif last_15["close"] < last_15["vwap"]:
        short_score += 4
        long_score -= 4
        short_reasons.append("VWAP موافق شورت؛ +4")
        long_reasons.append("VWAP مخالف لانگ؛ -4")

    # 6) Overall market direction from the same EMA basket: small soft bias only (±3).
    bullish_count = sum(1 for x in trends.values() if x == "bullish")
    bearish_count = sum(1 for x in trends.values() if x == "bearish")
    if bullish_count >= 3 and bullish_count > bearish_count:
        long_score += 3
        short_score -= 3
        long_reasons.append("روند کلی بازار/تایم‌فریم‌ها متمایل به لانگ؛ +3")
    elif bearish_count >= 3 and bearish_count > bullish_count:
        short_score += 3
        long_score -= 3
        short_reasons.append("روند کلی بازار/تایم‌فریم‌ها متمایل به شورت؛ +3")

    # 7) Simple quality guards: 1H fully opposite and 15M MACD opposite should not create entries.
    long_valid = adx_valid and trends.get("1H") != "bearish" and last_15["macd"] > last_15["macd_signal"]
    short_valid = adx_valid and trends.get("1H") != "bullish" and last_15["macd"] < last_15["macd_signal"]
    if trends.get("1H") == "bearish":
        long_reasons.append("رد لانگ: 1H خلاف جهت است")
    if trends.get("1H") == "bullish":
        short_reasons.append("رد شورت: 1H خلاف جهت است")
    if not (last_15["macd"] > last_15["macd_signal"]):
        long_reasons.append("رد لانگ: MACD 15M خلاف جهت است")
    if not (last_15["macd"] < last_15["macd_signal"]):
        short_reasons.append("رد شورت: MACD 15M خلاف جهت است")
