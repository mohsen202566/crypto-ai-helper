...
class AIController:
    ...
    def analyze(self, data: AnalysisInput) -> SignalDecision:
        ...
        if dir1h.state not in ("LONG", "SHORT"):
            return self._reject(
                ...
                reason="1H خنثی است؛ جهت اصلی برای شکار تایید نشد.",
                code="1H_NEUTRAL",
        ...
        cost = self.cost_engine.evaluate(
            direction=direction,
            entry=entry,
            tp=risk.tp,
            margin_usdt=self.storage.margin_usdt(),
            leverage=self.storage.leverage(),
            min_profit_usdt=self.storage.min_profit_usdt(),
            min_profit_pct=self.storage.min_profit_pct(),
        )
        ...
        watch_score = max(0, min(100, total - score_risk + min(6, max(0, risk.score))))
        notes = (
            dir1h.reasons
            + bias4h.reasons
            + pre.reasons
        ...
        if not risk.ok or not cost.ok:
            return self._reject(
                ...
                sl=risk.sl,
                breakdown=breakdown,
                reason="TP/SL، Net Edge، حداقل سود دلاری یا درصد سود قابل قبول نیست.",
                code="RISK_OR_PROFIT",
                direction=direction,
                hard=True,
                direction_state_1h=dir1h.state,
        ...
        return SignalDecision(
            ...
            threshold=SIGNAL_THRESHOLD,
            breakdown=breakdown,
            reason="سیگنال شکار معتبر است." if accepted else "امتیاز یا کندل شکار به حد نهایی نرسید.",
            reject_code=None if accepted else "LOW_SCORE_OR_NO_IGNITION",
            hunter=candle.label == "IGNITION_START",
            signal_label="شکار" if candle.label == "IGNITION_START" else "عادی",
            direction_state_1h=dir1h.state,
            direction_confidence_1h=dir1h.confidence,
            bias_4h=bias4h.state,
            setup_15m=pre.state,
    ...
    def _watch_candidate(self, watch_mode: bool, pre_score: int, watch_score: int, candle_label: str, ignition_state: str) -> bool:
        if candle_label == "EXHAUSTION":
            return False
        if ignition_state == "PRE_WATCH" or candle_label == "PRE_IGNITION_WATCH":
            return True
        if pre_score >= 7 and watch_score >= WATCH_THRESHOLD - 8:
            return True
        if watch_mode and pre_score >= 5 and candle_label != "NOISE":
            return True
        return False
    ...
    def _reject(
        ...
        *,
        ...
        sl: float = 0.0,
        hard: bool = False,
        direction_state_1h="NEUTRAL",
        direction_confidence_1h: int = 0,
        bias_4h="NEUTRAL",
        setup_15m="NEUTRAL",
        entry_5m="NO_ENTRY",
        candle_pattern="NOISE",
        ...
        return SignalDecision(
            ...
            hard_reject=hard,
            reject_code=code,
            direction_state_1h=direction_state_1h,
            direction_confidence_1h=direction_confidence_1h,
            bias_4h=bias_4h,
            setup_15m=setup_15m,
            entry_5m=entry_5m,
            candle_pattern=candle_pattern,
            entry_stage_pct=entry_stage_pct,
