...
def round_trip_fee_usdt(margin_usdt: float, leverage: int) -> float:
    """Estimated futures taker fee for entry + exit.

    This is the accounting fee shown in panels/results. Slippage buffer remains
    only a safety buffer for expected-edge checks.
    """
    return max(0.0, margin_usdt * leverage) * (TAKER_FEE_RATE * 2.0)


def gross_pnl_for_exit(margin_usdt: float, leverage: int, direction: str, entry: float, exit_price: float) -> float:
    return margin_usdt * leverage * direction_profit_pct(direction, entry, exit_price)


def net_pnl_for_exit(margin_usdt: float, leverage: int, direction: str, entry: float, exit_price: float) -> float:
    """Approximate net PnL after futures taker fees for both TP and SL.

    If gross is negative, subtracting the fee correctly makes the loss larger.
    """
    gross = gross_pnl_for_exit(margin_usdt, leverage, direction, entry, exit_price)
    return gross - round_trip_fee_usdt(margin_usdt, leverage)


def pnl_breakdown_for_exit(margin_usdt: float, leverage: int, direction: str, entry: float, exit_price: float) -> dict[str, float]:
    gross = gross_pnl_for_exit(margin_usdt, leverage, direction, entry, exit_price)
    fee = round_trip_fee_usdt(margin_usdt, leverage)
