...
class RuntimeSafety4H:
    """Crash-safe runtime laws for the simple 4H bot.

    Important: SLOT_RECHECK_SECONDS means exactly what the user requested:
    when local slots are full, wait 70 seconds, check Toobit, and only free a slot
    if Toobit no longer has that open position. It is not a generic sleep law.
    """
