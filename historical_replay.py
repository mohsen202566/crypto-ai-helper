...
from pnl_utils import net_profit_for_move
from utils import direction_profit_pct, now_utc
    ...
    def run_symbol(self, symbol: MarketSymbol, days: int = REPLAY_DAYS) -> ReplayResult:
        ...
        for index in range(220, len(candles) - 36, 6):
            ...
            if decision.accepted and decision.direction:
                ...
                net_result = "TP" if net.net_usdt > 0 else "SL"
                self.storage.record_observation(
                    source="replay",
                    signal_id=None,
                    features_key=decision.features_key,
                    symbol_name=symbol.name,
                    direction=decision.direction,
                    result=net_result,
                    net_profit=net.net_usdt,
                    mfe_pct=mfe,
                    mae_pct=mae,
                    tp_distance_pct=decision.tp_distance_pct,
                    sl_distance_pct=decision.sl_distance_pct,
                    reason=f"HISTORICAL_REPLAY_NET price_result={result} cost={net.cost_usdt:.6f}",
                )
                observations += 1
            else:
