...
class SignalMonitor:
    ...
    def _approx_pnl(signal: StoredSignal, exit_price: float) -> float:
        # Store approximate PnL as NET after round-trip taker fees. For SL this makes
        # the loss larger by the fee; for TP it subtracts the fee from profit.
        return net_pnl_for_exit(signal.margin_usdt, signal.leverage, signal.direction, signal.entry, exit_price)
