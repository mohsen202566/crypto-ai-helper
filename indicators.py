...
class IndicatorSnapshot:
    ...
    def ema200_slope_pct(self) -> float:
        return (self.ema200 - self.prev_ema200) / self.prev_ema200 if self.prev_ema200 > 0 else 0.0
...
def calculate_indicators(candles: list[Candle]) -> IndicatorSnapshot:
    ...
    vwap = _rolling_vwap(candles, 24)
    bb_mid, bb_upper, bb_lower, bb_width = _bollinger(closes, 20, 2.0)

    last = _last_complete_index(ema20, ema50, rsi, macd_line, macd_signal, macd_hist, adx, plus_di, minus_di, atr, vwap, bb_mid, bb_upper, bb_lower, bb_width)
    prev = _previous_complete_index(last, ema20, ema50, rsi, macd_hist, adx, atr, bb_width)
    ...
    window = candles[max(0, last - 80 + 1): last + 1]
    ...
    bb_range = max(float(bb_upper[last]) - float(bb_lower[last]), closes[last] * 0.0001)
    bb_position = (closes[last] - float(bb_lower[last])) / bb_range
    ...
    return IndicatorSnapshot(
        ...
        vwap=float(vwap[last]), bb_mid=float(bb_mid[last]), bb_upper=float(bb_upper[last]), bb_lower=float(bb_lower[last]),
        bb_width_pct=float(bb_width[last]), bb_position=float(bb_position), recent_high=float(recent_high), recent_low=float(recent_low),
        candle_range=float(candle_range), body_pct=float(body_pct), upper_wick_pct=float(upper_wick_pct), lower_wick_pct=float(lower_wick_pct),
        consecutive_up=_consecutive(candles[: last + 1], up=True), consecutive_down=_consecutive(candles[: last + 1], up=False),
    )
...
def _adx_dmi(highs: list[float], lows: list[float], closes: list[float], period: int) -> tuple[list[float | None], list[float | None], list[float | None]]:
    ...
    if len(valid_dx) < period:
        return adx, plus_di_out, minus_di_out
    compact_adx = _ema([float(v) for v in valid_dx], period)
    valid_indices = [i for i, v in enumerate(dx_values) if v is not None]
    for compact_i, value in enumerate(compact_adx):
        if value is not None:
            adx[valid_indices[compact_i]] = value
    return adx, plus_di_out, minus_di_out
...
def _bollinger(values: list[float], period: int, mult: float) -> tuple[list[float | None], list[float | None], list[float | None], list[float | None]]:
    mid: list[float | None] = [None] * len(values)
    upper: list[float | None] = [None] * len(values)
    lower: list[float | None] = [None] * len(values)
    width_pct: list[float | None] = [None] * len(values)
    for i in range(period - 1, len(values)):
        window = values[i - period + 1:i + 1]
        avg = sum(window) / period
        var = sum((v - avg) ** 2 for v in window) / period
        sd = math.sqrt(var)
        mid[i] = avg
        upper[i] = avg + mult * sd
        lower[i] = avg - mult * sd
        width_pct[i] = ((upper[i] - lower[i]) / values[i]) if values[i] > 0 else 0.0
...
def _consecutive(candles: list[Candle], *, up: bool) -> int:
    ...
    for c in reversed(candles):
        ok = c.close >= c.open if up else c.close <= c.open
        if ok:
            count += 1
        else:
            break
